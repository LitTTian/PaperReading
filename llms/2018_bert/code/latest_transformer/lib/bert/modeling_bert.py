...
class BertEmbeddings(nn.Module):
    """从词嵌入(word)、位置嵌入(position)和令牌类型(token_type)嵌入构造嵌入。"""
    """
    embeddings = word_embeddings + position_embeddings + token_type_embeddings
    词嵌入(word): 比如WordPiece嵌入 input_ids (B, seq_len, 1) -> word_embeddings (B, seq_len, hidden_size:768),
    位置嵌入(position): position_embeddings (B, seq_len, hidden_size:768)
    token类型嵌入(token_type): 句子A或句子B的类型 token_type_ids (B, seq_len) -> token_type_embedding (B, seq_len, hidden_size:768)
    # 调用nn.Embedding的过程实际上是一个查表的过程，将输入的id映射为对应的向量表示
    """

    def __init__(self, config):
        super().__init__()
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=config.pad_token_id)  # (vocab_size:30522, hidden_size:768)
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)  # (max_position_embeddings:512, hidden_size:768)
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size)  # (type_vocab_size: 2, hidden_size:768)
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor,  # (B, seq_len, hidden_size:768)
        ...
        query_layer = self.query(hidden_states)  # (B, seq_len, all_head_size:768)
        query_layer = query_layer.view(batch_size, -1, self.num_attention_heads, self.attention_head_size).transpose(
            1, 2
        )  # (B, num_heads:8, seq_len, head_size:96)

        is_updated = False
        is_cross_attention = encoder_hidden_states is not None  # HL: 交叉注意力层需要encode层的隐藏状态
        ...
        if is_cross_attention and past_key_values is not None and is_updated:
            # 复用k,v, cross_attentions
            # 比如在增量任务(如文本生成)中，不需要重复计算key和value，只需要用更新的query去计算注意力分数即可
            # NT: 因为来自encoder的encoder_hidden_states是不变的，推理的时候self.key()和self.value()也是不变的，所以只需要复用即可
        ...
        if attention_mask is not None:
            # 应用计算好的注意力掩码(在BertModel的forward()函数中为所有层预先计算)
