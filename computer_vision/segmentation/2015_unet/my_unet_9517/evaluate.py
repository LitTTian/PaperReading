...
def accuracy(model, val_loader, device, mapping): # used for validation
    ...
    ignore_index = 0
    with torch.no_grad():
        for batch in tqdm(val_loader, total=n_test, desc='Validation round', unit='imgs', leave=False):
            ...
            with torch.autocast(device.type if device.type != 'mps' else 'cpu'):
                ...
                ignore_index += (mask_true == 0).sum().item() # ignore background
    ...
    acc = total_correct / (total_pixels - ignore_index)
