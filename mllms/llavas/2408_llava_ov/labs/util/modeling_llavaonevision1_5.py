# 来自LLaVA-OneVision-1.5-8B-Instruct下的modeling_llavaonevision1_5.py
...
class LLaVAOneVision1_5_ModelOutputWithPast(ModelOutput):
    """
    llava输出的基类，包含隐藏状态和注意力。

    Args:
        last_hidden_state (`torch.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`):
            最后一层的隐藏状态序列。
        past_key_values (`tuple(tuple(torch.FloatTensor))`, *optional*, returned when `use_cache=True` is passed or when `config.use_cache=True`):
            元祖的元祖，`tuple(torch.FloatTensor)`，长度为`config.n_layers`，每个（内部）元组包含2个形状为
            `(batch_size, num_heads, sequence_length, embed_size_per_head)`的张量。

            该参数包含预计算的隐藏状态（即自注意力模块中的键（Key）和值（Value）张量），可通过该参数（参考
            `past_key_values` 输入参数说明）提升序列解码的速度。
...
class LLaVAOneVision1_5_RMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        LLaVAOneVision1_5_RMSNorm 和 T5LayerNorm 等价
        """
        super().__init__()
        self.weight = nn.Parameter(torch.ones(hidden_size))  # 可学习参数，形状为 (4096, )，初始化为全1
        self.variance_epsilon = eps  # 默认1e-6
...
class LLaVAOneVision1_5_MLP(nn.Module):
    # NT: hidden_size(4096) -> intermediate_size(12288) -> hidden_size(4096)
    # NT: x通过gate_proj升维激活后与x通过up_proj升维逐元素相乘，再通过down_proj降维
        ...
        **kwargs,
        ...
        Args:
            ...
            kwargs (`dict`, *optional*):
                ...
                into the model
        """
        residual = hidden_states  # (batch, seq_len, hidden_size)
        hidden_states = self.input_layernorm(hidden_states)  # 输入归一化

        # Decoder-Only的自注意力，QKV均来自hidden_states
        ...
        hidden_states = self.mlp(hidden_states)  # hidden_size(4096) -> intermediate_size(12288) -> hidden_size(4096)
        hidden_states = residual + hidden_states  # 残差连接
...
class LLaVAOneVision1_5_TextModel(Qwen2VLPreTrainedModel):
    ...
    def __init__(self, config: LLaVAOneVision1_5_TextConfig):
        ...
        self.layers = nn.ModuleList(
            [LLaVAOneVision1_5_DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]  # 36
        )
